...
(function () {
  ...
  function renderFeatured(post) {
    ...
    var excerpt = (post.excerpt || '').replace(/<[^>]*>/g, '').slice(0, 220);
  ...
  function renderGrid(posts) {
    ...
    grid.innerHTML = posts.map(function (p) {
      ...
      var date = p.published_at
        ...
        : '';
      var excerpt = (p.excerpt || '').replace(/<[^>]*>/g, '').slice(0, 200);
      ...
      return '<a href="/blog-post.html?slug=' + encodeURIComponent(p.slug) + '" class="post-card" data-cat="' + esc(cat) + '">'
        ...
        + '<div class="post-meta">' + (p.author_name ? esc(p.author_name) + ' · ' : '') + esc(date) + '</div>'
